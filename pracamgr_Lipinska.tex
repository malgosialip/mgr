...
Implementacja testu pierwszoœci Fermata w programie Matlab przedstawiona zosta³a w skrypcie \ref{testFermata}. W tabeli \ref{tabelaFermat} przedstawione s¹ wyniki dzia³ania tego kodu dla ró¿nych liczb pierwszych i z³o¿onych oraz czas dzia³ania algorytmu.
...
Test Fermata zastosowany do liczby Carmichaela jest nieskuteczny, co pokazane jest w tabeli \ref{tabelaFermatCarm}.
...
\textit{Test Lehmana} \cite[\mbox{s.} 60]{karbowski} \index{test pierwszoœci!Lehmana} jest kolejnym testem probabilistycznym pierwszoœci liczb. Algorytm testu Lehmana dla du¿ej liczby $p$ wygl¹da nastêpuj¹co:
\begin{enumerate}
\item Losujemy du¿¹ liczbê $a < p$.
\item Obliczamy $b=a^{(p-1)/2} \ (mod \ p)$.
