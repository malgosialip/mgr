...
\textit{Liczbami Mersenne'a} \index{liczby!Mersenne'a} s¹ liczby postaci $M_{n}=2^{n}-1$, $n \geq 1$. Je¿eli liczba Mersenne'a jest pierwsza to nazywa siê \textit{liczb¹ pierwsz¹ Mersenne'a} \index{liczby!pierwsze Mersenne'a}.
\end{definicja}
...
\end{align*}
i zredukowaæ nowe wspó³czynniki modulo $q$ tak, aby pozosta³o wyra¿enie w formie $a + b \sqrt{3}$. Dla zbioru $X$ zachodzi to¿samoœæ $1 = 1 + 0 \cdot ?3$. Zatem $X^{?}$, czyli zbiór odwracalnych elementów zbioru $X$, równie¿ jest grup¹. 
...
wiêc w zbiorze $X$ równie¿ równa jest $0$. Wtedy równania \ref{R1} i \ref{R2} mo¿emy zapisaæ w postaci
\begin{equation}
\label{R4}
...
\end{equation}
Kongruencjê \ref{R3} mo¿na przedstawiæ w postaci $\omega \cdot \omega^{2^{p-1}} \equiv 1 \ (mod \ q)$, sk¹d wynika, ¿e $\omega$ ma element odwrotny, a wiêc nale¿y do $X^{*}$.
...
Jednak¿e, jeœli $t < p$, to k mo¿e byæ równe $p-1$, wiêc $\omega^{2^{p-1}} \equiv 1 \ (mod \ q)$, co jest sprzecznoœci¹ z równaniem \ref{R4}. Zatem, poniewa¿ rz¹d $\omega$ jest mniejszy lub równy $2^{p}$ i~równoczeœnie wiêkszy lub równy $2^{p}$, wiêc musi byæ równy $2^{p}$. Rz¹d elementu nie mo¿e byæ wiêkszy od rzêdu grupy, wiêc $2^{p} \leq q^{2}-1$, ale $q^{2} \leq M_{p}$, wiêc  $q^{2} -1 \leq M_{p} - 1$, sk¹d $2^{p} \leq M_{p} - 1$. $M_{p}$ ma postaæ $2^{p} - 1$, wiêc $M_{p} - 1 = 2^{p} - 2$. Podsumowuj¹c, otrzymujemy $2^{p} \leq 2^{p} - 2$, co jest sprzecznoœci¹. St¹d za³o¿enie, ¿e liczba $q$ jest dzielnikiem $M_{p}$, jest b³êdne, wiêc $M_{p}$ musi byæ liczb¹ pierwsz¹. 
\end{dowod}
...
Trzeba zaznaczyæ, ¿e kongruencja \ref{mtfw} mo¿e zachodziæ tak¿e w pewnych przypadkach, gdy liczba $p$ nie jest liczb¹ pierwsz¹.
...
\end{enumerate}
Po zastosowaniu algorytmu liczba 561 zosta³a b³êdnie wskazana jako liczba prawdopodobnie pierwsza. Sta³o siê tak, poniewa¿ dla liczby Carmichaela równoœæ z ma³ego twierdzenia Fermata \ref{mtf} zachodzi dla wszystkich $a$ takich, ¿e $NWD(a, p) = 1$.
...
\end{equation}
to $n$ jest \textit{pseudopierwsz¹ liczb¹ Eulera} \index{liczby!pseudopierwsze Eulera} przy podstawie $b$.
\end{definicja}
...
%\index{test pierwszoœci!chiñski}
