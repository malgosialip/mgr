...
\begin{definicja}
\textit{Liczb¹ pierwsz¹} \index{liczby!pierwsze} nazywa siê liczbê naturaln¹ wiêksz¹ od 1, która ma dok³adnie dwa dzielniki: jedynkê i sam¹ siebie. 
\end{definicja}
...
\begin{definicja}
\textit{Najmniejsza wspólna wielokrotnoœæ} \index{najmniejsza wspólna wielokrotnoœæ} dla zadanych liczb ca³kowitych to najmniejsza liczba naturalna, któr¹ dzieli siê bez reszty przez ka¿d¹ z tych liczb. 
\end{definicja}

Najmniejsz¹ wspóln¹ wielokrotnoœæ liczb $a$ i $b$ oznacza siê symbolem $NWW(a,b)$, np. $NWW(2, 15) = 30$.

\begin{definicja}
\textit{Najwiêkszy wspólny dzielnik} \index{najwiêkszy wspólny dzielnik} dla zadanych dwóch lub wiêcej liczb ca³kowitych to najwiêksza liczba naturalna, która dzieli ka¿d¹ z tych liczb.
\end{definicja}

Najwiêkszy wspólny dzielnik liczb $a$ i $b$ oznaczany jest symbolem $NWD(a,b)$, np. $NWD(18, 15) = 3$.

\begin{definicja}
\textit{Liczby wzglêdnie pierwsze} \index{liczby!wzglêdnie pierwsze} to liczby ca³kowite, których najwiêkszym wspólnym dzielnikiem jest liczba $1$.
\end{definicja}
...
\begin{dowod} 
\begin{enumerate}
\item Wystarczy zauwa¿yæ, ¿e $a-a=0$ jest podzielna przez ka¿de $n \in \mathbb{N}$.
\item Je¿eli $a-b=kn$, gdzie $k \in \mathbb{Z}$, to $b-a=-kn$. St¹d wynika, ¿e $a \equiv b \ (mod \ n)$. 
\item Je¿eli $(a \equiv b \ (mod \ n)$ i $b \equiv c \ (mod \ n)$, to $a-b \equiv 0 \ (mod \ n)$ i~$b-c \equiv 0 \ (mod \ n)$. Opieraj¹c siê na to¿samoœci $$a-c = (a-b)+(b-c)$$ otrzymujemy, ¿e $a-c \equiv 0 \ (mod \ n)$, sk¹d wynika, ¿e $a \equiv c \ (mod \ n)$.
\end{enumerate} 
\textcolor{white}{...}
\end{dowod}
...
\textit{Odwrotnoœci¹ elementu} $a$ \textit{modulo} \index{odwrotnoœæ elementu modulo} $n$ jest taki element, oznaczany przez $a^{-1}$, ¿e
$$aa^{-1} \equiv 1 \ (mod \ n).$$
\end{definicja}
...
Szczególnym przypadkiem twierdzenia Eulera jest ma³e twierdzenie Fermata.
...
Jak wczeœniej zauwa¿ono, ci¹g liczb \eqref{ciag} modulo $p$ to ci¹g $1,2,...,p-1$, wiêc 
...
$$b^{n-1} \equiv 1 \ (mod \ n),$$ 
to $n$ jest \textit{liczb¹ pseudopierwsz¹} \index{liczby!pseudopierwsze} przy podstawie $b$.
\end{definicja}

Z twierdzenia \eqref{mtf} wynika, ¿e dla ka¿dej liczby $a$, która jest wzglêdnie pierwsza z modu³em $n$, istnieje liczba naturalna $k$ taka, ¿e $a^{k} \equiv 1 \ (mod \ n)$. 
...
\begin{definicja} \cite{lpb}
\textit{Liczba pierwsza Sophie Germain} \index{liczby!pierwsze Sophie Germain} to dowolna liczba pierwsza $p$, dla której liczba $2p+1$ tak¿e jest liczb¹ pierwsz¹ (np. 23, gdy¿ $2 \cdot 23 +1 = 47$).
\end{definicja}
...
\begin{dowod}
Równanie \eqref{lematAKS} mo¿na zapisaæ w postaci
$$(X+a)^{n}-(X^{n}+a)=0 \ (mod \ n).$$
...
\item $(log_{2} n)^{2} \approx 11,97$, wiêc $r = 13$, poniewa¿ $o_{13}(11) = 12$.
...
\item $d = NWD(4,23) = 1$. 
\item Kongruencja $a^{p-1} \equiv 1 \ (mod \ p)$ jest spe³niona, poniewa¿ $4^{22} \equiv 1 \ (mod \ 23)$.
...
\item $d = NWD(8,25) = 1$. 
\item Kongruencja  $a^{p-1} \equiv 1 \ (mod \ p)$ nie zachodzi, poniewa¿ $8^{24} \equiv 21 \ (mod \ 25)$.
...
\textit{Liczby Carmichaela} \index{liczby!Carmichaela} to z³o¿one liczby naturalne, dla których teza ma³ego twierdzenia Fermata (twierdzenie \ref{mtf}) jest prawdziwa. Liczba naturalna $n$ jest liczb¹ Carmichaela wtedy i~tylko wtedy, gdy jest liczb¹ z³o¿on¹ i~dla ka¿dej liczby naturalnej $a$ z przedzia³u $1 < a < n$, wzglêdnie pierwszej z $n$, liczba $a^{n-1}-1$ jest podzielna przez $n$.
\end{definicja}
...
\item Obliczamy $b = a^{(31-1)/2} = a^{15} \equiv 30 \ (mod \ 31)$.
...
Symbol Legendre'a mo¿na obliczyæ korzystaj¹c z nastêpuj¹cego wzoru
...
\begin{definicja}
Nieparzyst¹ liczbê z³o¿on¹ $n$ nazywamy \textit{pseudopierwsz¹ liczb¹ Eulera} \index{liczby!pseudopierwsze Eulera} przy podstawie $b$, gdzie $(n,b)=1$, jeœli spe³niony jest warunek
