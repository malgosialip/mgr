...
\begin{definicja}
\textit{Najmniejsza wspólna wielokrotnoœæ} dla zadanych liczb ca³kowitych to najmniejsza liczba naturalna, któr¹ dzieli siê bez reszty przez ka¿d¹ z tych liczb. Zapisuje siê to jako $NWW(a,b)=c$, np. $NWW(2, 15) = 30$.
\end{definicja}

\begin{definicja}
\textit{Najwiêkszy wspólny dzielnik} dla zadanych dwóch lub wiêcej liczb ca³kowitych to najwiêksza liczba naturalna, która dzieli ka¿d¹ z tych liczb. Zapisuje siê to jako $NWD(a,b)=c$, np. $NWD(18, 15) = 3$.
\end{definicja}
...
Do zrozumienia dzia³ania powy¿szego algorytmu nale¿y wprowadziæ pojêcie reszty z dzielenia.
...
Do znalezienia wszystkich liczb pierwszych mniejszych od jakiejœ liczby wystarczy \textit{odsiaæ} liczby z³o¿one i jedynkê. Pierwszy s³u¿¹cy do tego algorytm zosta³ przedstawiony przez Eratostenesa z Cyreny (III-II w. p.n.e.), a póŸniej ulepszony przez Arthura Atkina i D.J. Bernsteina.

\begin{definicja} \cite[\mbox{s.} 48-49]{karbowski}
\label{sito}
Najprostszym i jednoczeœnie najmniej wydajnym algorytmem wyszukiwania kolejnych liczb pierwszych z przedzia³u $[2,n]$ jest tak zwane \textit{Sito Eratostenesa}. Algorytm polega na usuwaniu liczb z³o¿onych z zadanego zbioru liczb naturalnych w taki sposób, ¿e ze zbioru wybieramy najmniejsz¹ liczbê i usuwamy wszystkie jej wielokrotnoœci, nastêpnie wybieramy najmniejsz¹ z pozosta³ych i postêpujemy tak jak z pierwsz¹. Na koniec procesu w zbiorze pozostan¹ tylko liczby pierwsze. 
\end{definicja}
...
Sito Eratostenesa mo¿na teoretycznie zapisaæ w postaci funkcji, na przyk³ad w programie Matlab (skrypt \ref{sitoEratostenesa}). Poni¿ej przedstawiono tabelê z wynikami przedzia³ów o ró¿nej d³ugoœci.
...
\begin{definicja} \cite{sitoAtkina}
...
\textit{Spirala Ulama} lub \textit{spirala liczb pierwszych} to graficzna metoda przedstawiania rozk³adu liczb pierwszych. Sposób zapisu polega na tym, ¿e na kwadratowej tablicy zaczynaj¹c od 1 w œrodku spiralnie wypisuje siê kolejne liczby naturalne. Okazuje siê, ¿e na pewnych przek¹tnych liczby pierwsze pojawiaj¹ siê o wiele czêœciej ni¿ na innych. Co wiêcej, zjawisko nie zale¿y od wyboru pierwszej liczby w spirali. Na Rysunku \ref{ulam} przedstawiono spiralê Ulama o rozmiarze $200$x$200$.
\begin{figure}[H]
\label{ulam}
\begin{center}
...
Podrozdzia³ ten jest poœwiêcony dwóm klasycznym twierdzeniom z zakresu teorii liczb - twierdzeniu Wilsona i ma³emu twierdzeniu Fermata. Maj¹ one szerokie zastosowanie w testach pierwszoœci liczb. Do ich omówienia potrzebne bêd¹ pojêcia takie jak kongruencja, zbiór reszt modulo i odwrotnoœæ elementu, które zosta³y zdefiniowane poni¿ej.  

\begin{definicja}  \cite[\mbox{s.} 13]{zakrzew}
...
$$a \equiv b \  mod \  n.$$
Zapis ten nazywany jest kongruencj¹.
\end{definicja}
...
\item Jeœli $p_{1}, p_{2},...,p_{k}$ s¹ wszystkimi czynnikami pierwszymi liczby $n$ bez powtórzeñ, to $$\varphi(n) = n \left(1-\frac{1}{p_{1}}\right)\left(1-\frac{1}{p_{2}}\right)...\left(1-\frac{1}{p_{k}}\right).$$
\end{enumerate}
...
$$\num{18543637900515} \cdot 2^{\num{666667}}-1,$$ a jej zapis dziesiêtny wymaga $\num{200701}$ cyfr. Zosta³a ona odkryta w 2012 roku przez Philipa Bliedunga.
...
\hline
\end{tabular}
\caption{Dziesiêæ najwiêkszych znanych liczb pierwszych na 2018 rok \cite{gimps}.}
\label{toptengreatestprimenumbers}
\end{center}
\end{table}

\section{"Porz¹dek mno¿¹cy"}
...
$$a^{k} \equiv 1 \ mod \ n.$$
Porz¹dek modulo $n$ oznacza siê $ord_{n}(a)$ lub $o_{n}(a)$. 
\end{definicja}

\textit{Multiplicative order} jest równie¿ bardzo prosty do zaimplementowania w œrodowisku Matlab (skrypt \ref{multorder}). Poni¿ej przedstawiono tabelê z wynikami i czasem dzia³ania powy¿szej funkcji.
...
\begin{definicja} \cite{liczbaCarm}
...
\begin{center}
\label{sitoEratostenesa}
\lstinputlisting[frame=single]{sitoEratostenesa.m}
\end{center}
%\caption{Skrypt funkcji Sito Eratostenesa}
...
\end{center}
%\caption{Skrypt funkcji Eulera}

\section*{Multiplicative order}
\begin{center}
\label{multorder}
\lstinputlisting[frame=single]{mult_order.m}
\end{center}
%\caption{Skrypt funkcji multiplicative order}
...
\end{center}
%\caption{Skrypt testu Solovaya-Strassena}
...
\end{center}
%\caption{Skrypt testu Millera-Rabina}
